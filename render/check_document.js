...
function check_document_start(data)//文档校验开始函数
{
    ...
    */
    ...
    if(parameter!="")
    {
        ...
        try {
            let stdout=execSync("python ../render/word-transform.py "+parameter,childoption);
    ...
    */
    if(filepath.length>0)
    {
        ...
        try {
            request_delay();
            let stdout=execSync("python ../render/word-compare.py "+parameter,childoption);
            request_empty_tip();
            if(stdout==200)
            {
...
function compare_result_display(filepath)//将返回的json数据呈现在校准页面上。
{
    ...
    content_http.addEventListener("load",()=>
    {
        ...
        fs.unlinkSync(path.join(__dirname,"\\..\\data\\",str));//清除docx文档生成的临时数据
    });
