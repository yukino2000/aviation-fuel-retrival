...
def compare_docx(main_data):#校验文档
    ...
    main_keycp=["cnname","stdno","issue_time","act_time","state"]
    cmp_result=[]
    for item in range(2,len(filelist)):
        ...
        for i in docx_data:
            ...
            for ex in i:
                cmp_key=""
                for a in range(0,len(main_key)):#看这个docx_data里关键字主库里有没有，如果有则进一步比较
                    if ex==main_key[a]:
                        cmp_key=main_keycp[a]
                        break
...
def writeresult(docx_data,cmp_result,path):#将比较结果写成docx文档
    ...
    #再将数据打包成json文件，供软件呈现
    json_data=[]
    for i in range(2,len(docx_data)):
        rows={}
...
def readdata():#读取主库，与主库相比
    ...
    for item in res:
        if item.startswith("{"):
            main_data.append(json.loads(item))
