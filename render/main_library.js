...
{
    if(!fs.existsSync(path.join(userdir,"temp_data.json")))//当检查发现本地不存在数据时，则需要从向主机请求数据
    {
...
function main_mysqlrequestdata()//请求数据函数，该函数连接mysql服务器请求数据
{
    ...
    fs.writeFileSync(path.join(userdir,"temp_data.json"));
    let ws=null;
      //发生错误时
    ...
    constream.on("packet",(packet)=>{
        request_delay();
        ws=fs.createWriteStream(path.join(userdir,"temp_data.json"));
        console.log(packet);
    });
...
{
    let rs=fs.readFileSync(path.join(userdir,"temp_data.json"),{encoding:"utf-8",flag:"r"});
    rs=rs.split("#,#");
