...
{
    if(!fs.existsSync(path.join(__dirname,"..\\data\\temp_data.json")))//当检查发现本地不存在数据时，则需要从向主机请求数据
    {
        main_json_request=main_mysqlconnection();
        globaltimeout.push(debounce(main_mysqlrequestdata,1000).run());
    }//当存在本地数据时，则直接请求本地数据即可。
    else
    {
        main_localrequestdata();
        search_result();
    }
...
function main_mysqlrequestdata()//请求数据函数，该函数连接mysql服务器请求数据
{
    let constream=main_json_request.query("select * from ??",["mainlibview"]);
    fs.writeFileSync(path.join(__dirname,"..\\data\\temp_data.json"));
    ...
    constream.on("packet",(packet)=>{
        request_delay();
        ws=fs.createWriteStream(path.join(__dirname,"..\\data\\temp_data.json"));
        console.log(packet);
    });
    ...
    constream.on("result",(row)=>{
        ...
        ws.write(JSON.stringify(row).toString()+"#,#");
...
function main_localrequestdata()//请求数据函数，该函数请求本地数据
{
    let rs=fs.readFileSync(path.join(__dirname,"..\\data\\temp_data.json"),{encoding:"utf-8",flag:"r"});
    rs=rs.split("#,#");
    for(let a=0;a<rs.length;a++)
        if(rs[a].indexOf("{")!=-1)
        {
            main_json_data.push(JSON.parse(rs[a]));
        }
}
